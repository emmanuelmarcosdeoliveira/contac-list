  ...
  return (
    <S.CarddeContatos>
      ...
      <S.MyDiv>
        ...
        <S.Email
        disabled={!editando}
        value={email} onChange={(evento) => setEmail(evento.target.value)}>{email}</S.Email>
      </S.MyDiv>
      <S.MyDiv>
        <FaPhoneAlt style={{ fontSize: '0.75rem', marginRight: '1rem' }} />
        <S.TelPhone 
        disabled={!editando}
        value={telefone} onChange={(evento) => setTelefone(evento.target.value)}>{telefone}</S.TelPhone>
      </S.MyDiv>
      <S.ActionBar>
    {editando ? (
      <>
